...
Tauri 릴리즈 요청에서 대상은 반드시 `/Users/terecal/pilot-project/sample-pkt-project/tikitaka-note/pkt-study-fullstack` 저장소로 한다. 이 저장소의 `.data/pkt-study.db`를 기준 시드로 확인·포함하고, `design-playbook-tauri/`는 별도 실험용 중첩 저장소이므로 사용자의 명시적 요청 없이는 커밋·푸시·릴리즈하지 않는다.

...
import { AccordionPractice } from "./AccordionPractice";
...
export function MissionDetail() {
  ...
  const hasPreview = mission.id === "accordion";
        ...
        <section className="mx-auto max-w-5xl">
          ...
          <article className="mt-5 rounded-2xl border border-border bg-card p-6 shadow-sm sm:p-10">
            ...
            <div className="mt-10 rounded-xl border border-dashed border-border bg-muted/30 p-5">
              ...
              <p className="mt-2 text-sm leading-6 text-muted-foreground">실습 구현을 시작하면 요구사항, 컴포넌트 상태, 접근성 항목과 검증 포인트를 이 페이지에 연결합니다.</p>
            </div>
            {hasPreview && <AccordionPractice />}
            {isReady && <Link href="/main" className="mt-8 inline-flex h-10 items-center gap-2 rounded-md bg-primary px-4 text-sm font-semibold text-primary-foreground hover:opacity-90">기본 UI 구현 시작 <ArrowRight className="h-4 w-4" /></Link>}
          </article>
        </section>
      </main>
  );

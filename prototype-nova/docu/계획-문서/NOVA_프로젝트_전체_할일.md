...
- [x] `POST /signup` API 계약 확정: 로그인 ID·비밀번호·표시명 검증, 201/400/409/503 오류 형식 정의
- [x] `users.password_hash`에 BCrypt 단방향 해시를 저장하고 로그인 ID UNIQUE 제약 적용
- [x] 활성 User와 기본 `CUSTOMER` 역할 관계를 하나의 트랜잭션으로 생성
- [x] 정상 가입, 중복 ID, 형식 오류, 기본 역할 누락 통합 테스트 및 `./gradlew test` 통과
- [x] 구현·검증 기록: `prototype-nova/nova-backend/docs/signup-api-spec.md`

...
public record LoginResponse(Long id, String email, String displayName, List<String> roleCodes) {
    public static LoginResponse from(User user, List<String> roleCodes) {
        return new LoginResponse(user.getId(), user.getEmail(), user.getDisplayName(), roleCodes);
    }
}

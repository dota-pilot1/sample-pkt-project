...
`POST /login`에서 정규화한 이메일로 사용자를 찾고 BCrypt로 비밀번호를 검증한다. 활성 사용자와 역할을 확인한 뒤 인증 결과를 발급한다. 인증 방식(JWT 등)이 정해지면 현재 공개인 `/api/**` 경로를 역할별 권한 정책으로 전환하고, 인증 실패는 401, 권한 부족은 403으로 구분한다.

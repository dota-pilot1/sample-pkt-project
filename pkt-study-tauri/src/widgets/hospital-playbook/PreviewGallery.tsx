    ...
    <div className="flex flex-col gap-3">
      {blocks.map((block, index) => (
        <div key={index} className="overflow-hidden rounded-lg border border-surface-border-soft bg-surface-raised">
          <div className="flex items-center gap-2 border-b border-surface-border-soft px-3 py-2">
            <span className="text-[12px] font-black text-text-primary">
              {block.label || `미리보기 ${index + 1}`}
            </span>
            <span className="rounded-full bg-surface-muted px-2 py-0.5 text-[10px] font-black text-text-muted">

...
export default function LoginPage() {
  ...
  const signupState = location.state as { signupMessage?: string; email?: string } | null;
  const [email, setEmail] = useState(signupState?.email ?? "terecal@daum.net");
  ...
  return <main className="grid min-h-screen place-items-center bg-slate-100 p-5">
    <section className="w-full max-w-md rounded-3xl border border-slate-200 bg-white p-8 shadow-sm">
      ...
      {signupState?.signupMessage && <p role="status" className="mb-4 rounded-xl bg-emerald-50 px-3 py-3 text-sm font-semibold text-emerald-700">{signupState.signupMessage}</p>}
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <Link to="/signup" className="block text-center text-sm font-bold text-sky-700 hover:text-sky-900">처음 방문하셨나요? 회원가입</Link>
      </form>
    </section>
  </main>;
}

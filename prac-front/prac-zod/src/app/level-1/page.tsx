...
function EyeIcon({ closed = false }: { closed?: boolean }) {
  return closed
    ? <svg aria-hidden="true" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="m3 3 18 18M10.6 10.6a2 2 0 0 0 2.8 2.8M9.9 4.2A10.8 10.8 0 0 1 12 4c5.2 0 8.7 4 10 8a12.8 12.8 0 0 1-3.1 5.1M6.2 6.2C3.9 7.7 2.6 10.1 2 12c1.3 4 4.8 8 10 8 1.3 0 2.5-.2 3.5-.7" /></svg>
    : <svg aria-hidden="true" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M2 12s3.5-8 10-8 10 8 10 8-3.5 8-10 8S2 12 2 12Z" /><circle cx="12" cy="12" r="3" /></svg>;
}
...
export default function HomePage() {
  ...
  const [visiblePasswords, setVisiblePasswords] = useState({ password: false, passwordConfirm: false });
  ...
  return <main className="shell">
    <header className="hero"><p className="eyebrow">FRONTEND PRACTICE · LEVEL 1</p><h1>기본 스키마</h1><p>Zod 스키마를 만들고 입력 데이터의 모양을 안전하게 확인합니다.</p></header>
    <section className="grid">
      ...
      <article className="card"><div className="card-title"><span>02</span><div><h2>회원가입 스키마</h2><p>입력값은 모두 처음에는 문자열입니다.</p></div></div><div className="form">
        {([['email','이메일','email'],['age','나이','text']] as const).map(([name, label, type]) => <label key={name}>{label}<input type={type} value={form[name]} onChange={(event) => setForm((current) => ({ ...current, [name]: event.target.value }))} placeholder={name === 'age' ? '예: 20' : undefined} /></label>)}
        {([['password','비밀번호'],['passwordConfirm','비밀번호 확인']] as const).map(([name, label]) => <label key={name}>{label}<span className="password-input"><input type={visiblePasswords[name] ? "text" : "password"} value={form[name]} onChange={(event) => setForm((current) => ({ ...current, [name]: event.target.value }))} /><button type="button" className="icon-button" aria-label={visiblePasswords[name] ? `${label} 숨기기` : `${label} 보기`} onClick={() => setVisiblePasswords((current) => ({ ...current, [name]: !current[name] }))}><EyeIcon closed={!visiblePasswords[name]} /></button></span></label>)}
